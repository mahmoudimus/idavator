...
class LLVMToMicrocodeConverter:
    ...
    def __init__(self):
        ...
        self.temp_counter = 0
        self.ptrsize = 64 if ida_ida.inf_is_64bit() else 32
    ...
    def parse_llvm_ir(self, ir_text: str) -> llvm.ModuleRef:
        ...
        try:
            ...
            logging.exception("Failed to parse LLVM IR: %s", e)
            raise
    ...
    def _convert_return(self, llvm_insn, mblock):
        ...
        if len(llvm_insn.operands) > 0:
            ...
            if result_lvar:
                store_insn = self._create_minsn(
                    hr.m_stx, self._get_operand_size(ret_val)
                )
    ...
    def _get_or_create_temp_lvar(self, llvm_insn):
        ...
        name = (
            llvm_insn.name
            if hasattr(llvm_insn, "name") and llvm_insn.name
            else f"tmp_{self.temp_counter}"
        )
        self.temp_counter += 1
